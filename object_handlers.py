...
def handlePassport(objects, im_height):
  ...
  money_objs = sorted(passport_nums_objs, key = lambda obj: obj['bbox'][3], reverse=True)

  passport_series_objs = list(filter(lambda obj: obj['field_type'] == 'PassportSeries', objects))
  money_objs = sorted(passport_series_objs, key = lambda obj: obj['bbox'][3], reverse=True)

  return passport_nums_objs + passport_series_objs
